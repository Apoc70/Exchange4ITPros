...
<#
    ...
    Version 3.0.1, 2026-09-17

    .DESCRIPTION

    This script creates an HTML report showing the following information about an Exchange
    Subscription Edition, 2019, 2016, 2013, 2010, and, to a lesser extent, 2007 and 2003 environments.
...
# Embedded resources
# Replaced automatically by Build\Build-Standalone.ps1

$EmbeddedCss = @'
/*
    Exchange Environment Report - Stylesheet 
    Refreshed color palette and typography, kept table-based and Outlook/Word rendering
    engine compatible since this file is also embedded inline when the report is sent
    as an HTML mail body. Avoid flexbox, grid, box-shadow and CSS variables here.
*/
...
table th.header-totalservers {
  background: #1f4e79;
}
table th.header-totalmailboxes {
  background: #2f6f4f;
}
table th.header-totalroles {
  background: #9a6328;
}
...
table tr.overviewcolheader {
  background: #1f4e79;
}
table tr.overviewcolsubheader {
  background: #3a6ea5;
}

table tr.pre2007overviewcolheader {
  background: #6b3fa0;
}
table tr.pre2007overviewcolsubheader {
  background: #8a5cc2;
}
...
/* Subheader, Exchange Server Versions, Org and roles */
table tr.subheader {
  background: #dfe7ee;
}
table th.subheader {
  font-family: 'Segoe UI', Verdana, Geneva, Arial, Helvetica, sans-serif, monospace, Lucida Console;
  color: #ffffff;
  font-size: 11px;
  text-align: center;
  padding: 4px 6px;
}
table th.subheader-totalservers {
  background: #3c6ea4;
}
table th.subheader-totalmailboxes {
  background: #4d8d69;
}
table th.subheader-totalroles {
  background: #c7873a;
}

/* Header data, Mailbox numbers in header table */
table tr.headerdata {
  background: #eef1f5;
}
...
.tooltip .tooltiptext::after {
  ...
  border-width: 5px;
  border-style: solid;
  border-color: #14233c transparent transparent transparent;
}
...
$EmbeddedVersionMappingJson = @'
{
  ...
  "MajorVersions": {
    ...
    "15.3": { "Long": "Exchange SE", "Short": "SE " }
  },
  "ServicePackLevels": {
    "0": "RTM",
    "1": "SP1",
  ...
  "SecurityUpdates": {
    ...
    "15.2.2562.43": "Jun26SU",
    "15.2.2562.41": "May26HU",
    "15.2.2562.37": "Feb26SU",
    "15.2.2562.35": "Dec25SU",
    "15.2.2562.29": "Oct25SU",
    ...
    "15.2.1748.46": "Jun26SU",
    "15.2.1748.43": "Feb26SU",
    "15.2.1748.42": "Dec25SU",
    "15.2.1748.39": "Oct25SU",
    "15.2.1748.37": "Sep25HU",
    "15.2.1748.36": "Aug25SU",
    ...
    "15.2.1544.41": "Jun26SU",
    "15.2.1544.39": "Feb26SU",
    "15.2.1544.37": "Dec25SU",
    "15.2.1544.36": "Oct25SU",
    "15.2.1544.34": "Sep25HU",
    "15.2.1544.33": "Aug25SU",
    "15.2.1544.27": "May25HU",
    "15.2.1544.25": "Apr25HU",
    "15.2.1544.14": "Nov24SUv2",
    ...
    "15.1.2507.69": "Jun26SU",
    "15.1.2507.66": "Feb26SU",
    "15.1.2507.63": "Dec25SU",
    "15.1.2507.61": "Oct25SU",
    "15.1.2507.59": "Sep25HU",
    "15.1.2507.58": "Aug25SU",
    "15.1.2507.57": "May25HU",
    "15.1.2507.55": "Apr25HU",
    "15.1.2507.44": "Nov24SUv2",
...
function Import-ExchangeVersionMapping {
  ...
  try {
    if (-not $Path -and -not $JsonContent) {
        throw ('Either a Path or JsonContent must be provided.')
    }
    if (-not $JsonContent) {
    ...
    $SourceName = if ($Path) { $Path } else { 'Embedded JSON' }
  }
  catch {
    throw ('Failed to read or parse version mapping file {0}: {1}' -f $SourceName, $_.Exception.Message)
  }
  ...
  @{
    MappingVersion = $JsonObject.MappingVersion
...
"@
  ...
  elseif (Test-Path -Path (Join-Path -Path (Split-Path -Parent $Path) -ChildPath $CssFileName)) {
  ...
  $Output += ("<h2 align=""center"">{5}</h2><h3 align=""center"">Organization: {3}</h3>
      <h4 align=""center"">Generated {0} - Script Version {4} - Mapping Version {6}</h4>
      <table class='header'>
      <tr class='header'>
  <th colspan=""{1}"" class='header header-totalservers'>{2}</th>" -f (Get-Date -Format $DateLabelFormat), $ExchangeEnvironment.TotalMailboxesByVersion.Count, $LabelTotalServers, $ExchangeEnvironment.OrganizationName, $ScriptVersion, $ReportTitle, $ExchangeEnvironment.MappingVersion)
...
if ($OutputFormat -ne 'Markdown') {
  if ($EmbeddedCss) {
    Write-Verbose "Using embedded CSS"
  }
  elseif (Test-Path -Path (Join-Path -Path $ScriptDir -ChildPath $CssFileName)) {
...
if ($EmbeddedVersionMappingJson) {
    Write-Verbose "Using embedded version mapping"
}
elseif (Test-Path -Path $VersionMappingFilePath) {
...
# Loaded from an external JSON file or embedded JSON content only to simplify updates whenever Microsoft releases a new
# Cumulative Update or Security Update, no script code changes are required for that case.
if ($EmbeddedVersionMappingJson) {
    $VersionMapping =
    Import-ExchangeVersionMapping -JsonContent $EmbeddedVersionMappingJson -Path 'Embedded'

    }
else {
    $VersionMapping =
    Import-ExchangeVersionMapping -Path $VersionMappingFilePath -JsonContent $null
}
...
$ExchangeEnvironment.Add('MappingVersion', $VersionMapping.MappingVersion)
